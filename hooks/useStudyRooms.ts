...
export function useStudyRooms() {
  ...
  useEffect(() => {
    ...
    const membersSubscription = supabase
      .channel(`study_room_members_${uniqueId}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
    ...
    const sessionsSubscription = supabase
      .channel(`study_sessions_${uniqueId}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
  ...
  const createRoom = async (name: string, description: string, isPrivate: boolean, password?: string) => {
    try {
      ...
      if (data) {
        await joinRoom((data as any).id, password);
      }

      // Manual olarak rooms listesini yenile (real-time subscription gecikmesi için)
      setTimeout(() => {
        fetchRooms();
      }, 500);
  ...
  const joinRoom = async (roomId: string, password?: string) => {
    try {
      ...
      // Şifre kontrolü
      if ((room as any).is_private) {
        if (!password) {
          throw new Error('Bu oda için şifre gerekli');
        }
        if (password !== (room as any).password_hash) {
